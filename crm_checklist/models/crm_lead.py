...
class crm_lead(models.Model):
    ...
    @api.depends("stage_id", "team_id", "check_list_line_ids", "check_list_line_ids.team_ids",
        ...
        "stage_id.requires_kyc",
        "stage_id.requires_crm",
        "stage_id.requires_cc",
        "stage_id.requires_cs",
        "stage_id.requires_vrm",
        "stage_id.requires_vc",
        "stage_id.requires_vs",
        )
    def _compute_check_list_len(self):
        ...
        for lead_id in self:
            ...
            # add partner application requirements
            check_list_len += 1 if stage_id.requires_kyc else 0
            check_list_len += 1 if stage_id.requires_crm else 0
            check_list_len += 1 if stage_id.requires_cc else 0
            check_list_len += 1 if stage_id.requires_cs else 0
            check_list_len += 1 if stage_id.requires_vrm else 0
            check_list_len += 1 if stage_id.requires_vc else 0
            check_list_len += 1 if stage_id.requires_vs else 0
            
            pa_req_checked = 0
            pa_req_checked += 1 if (stage_id.requires_kyc and lead_id.current_kyc_state == 'active') else 0
            pa_req_checked += 1 if (stage_id.requires_crm and lead_id.current_crm_state == 'active') else 0
            pa_req_checked += 1 if (stage_id.requires_cc and lead_id.current_customer_credit_state == 'active') else 0
            pa_req_checked += 1 if (stage_id.requires_cs and lead_id.current_customer_strategy_state == 'active') else 0
            pa_req_checked += 1 if (stage_id.requires_vrm and lead_id.current_vrm_state == 'active') else 0
            pa_req_checked += 1 if (stage_id.requires_vc and lead_id.current_vendor_credit_state == 'active') else 0
            pa_req_checked += 1 if (stage_id.requires_vs and lead_id.current_vendor_strategy_state == 'active') else 0

            check_list_line_ids = lead_id.check_list_line_ids._get_filtered(stage_id, team_id)
            lead_id.check_list_len = check_list_len
            lead_id.checklist_progress = check_list_len and ((pa_req_checked + len(check_list_line_ids)) / check_list_len) * 100 or 0.0
    ...
    checklist_progress = fields.Float(
        string="Checklist progress", compute=_compute_check_list_len, store=True, copy=False,
    )
