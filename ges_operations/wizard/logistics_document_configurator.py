...
class LogisticsDocConfig(models.TransientModel):
    ...
    shipment_order_id = fields.Many2one('shipment.order', string="Shipment Order", domain="[('partner_id','=',partner_id)]")
    transport_order_id = fields.Many2one('transport.order', string="Transport Order", domain="[('partner_id','=',partner_id)]")
    storage_order_id = fields.Many2one('storage.order', string="Storage Order", domain="[('partner_id','=',partner_id)]")
    customs_order_id = fields.Many2one('customs.order', string="Customs Order", domain="[('partner_id','=',partner_id)]")
    service_order_id = fields.Many2one('service.order', string="Service Order", domain="[('partner_id','=',partner_id)]")


    def apply_doc_config_wizard(self):
        if self.sale_order_line_workflow == "sho":
            if self.shipment_order_id:
                self.sol_id.reference_document = '%s,%s' % ('shipment.order',self.shipment_order_id.id)
            ...
            if self.transport_order_id:
                self.sol_id.reference_document = '%s,%s' % ('transport.order',self.transport_order_id.id)
            ...
            if self.storage_order_id:
                self.sol_id.reference_document = '%s,%s' % ('storage.order',self.storage_order_id.id)
                self.sol_id.name = self.sol_id.name + "\n" + "Storage Desc 1"
            else:
            ...
            if self.customs_order_id:
                self.sol_id.reference_document = '%s,%s' % ('customs.order',self.customs_order_id.id)
                self.sol_id.name = self.sol_id.name + "\n" + "Customs Desc 1"
            else:
            ...
            if self.service_order_id:
                self.sol_id.reference_document = '%s,%s' % ('service.order',self.service_order_id.id)
                self.sol_id.name = self.sol_id.name + "\n" + "Service Desc 1"
            else:
