...
class ShipmentOrder(models.Model):
    ...
    name_seq = fields.Char(string="Name Seq", copy=False, default=lambda self: ('New'))
    ...
    move_type = fields.Selection([('d2d','Door-to-Door'),('d2p','Door-to-Port'),('p2p','Port-to-Port'),('p2d','Port-to-Door')], required=True, default="p2p", string="Move Type")
    
    @api.depends('transport','move_type')
    @api.onchange('transport','move_type')
    def _update_address_types(self):
        if self.transport == 'road':
            self.origin_address_type = 'location'
            self.destination_address_type = 'location'
    ...
    def _reset_origin_address(self):
        if self.origin_address_type == 'port':
            self.origin_address_id = False
        elif self.origin_address_type == 'location':
    ...
    def _reset_destination_address(self):
        if self.destination_address_type == 'port':
            self.destination_address_id = False
        elif self.destination_address_type == 'location':
    ...
    pickup_address_type = fields.Selection([('port','Port'),('location','Location')])
    ...
    #pickup dates
    pickup_etd_datetime = fields.Datetime('PU-ETD', help="Estimated Time of Departure")
    pickup_atd_datetime = fields.Datetime('PU-ATD', help="Actual Time of Departure")
    pickup_eta_datetime = fields.Datetime('PU-ETA', help="Estimated Time of Arrival")
    pickup_ata_datetime = fields.Datetime('PU-ATA', help="Actual Time of Arrival")
    

    #delivery fields
    delivery_option = fields.Boolean(string="Delivery?")
    delivery_address_type = fields.Selection([('port','Port'),('location','Location')])
    ...
    #delivery dates
    delivery_etd_datetime = fields.Datetime('DL-ETD', help="Estimated Time of Departure")
    delivery_atd_datetime = fields.Datetime('DL-ATD', help="Actual Time of Departure")
    delivery_eta_datetime = fields.Datetime('DL-ETA', help="Estimated Time of Arrival")
    delivery_ata_datetime = fields.Datetime('DL-ATA', help="Actual Time of Arrival")
    ...
    # Carriage Dates
    etd_datetime = fields.Datetime('ETD', help="Estimated Time of Departure")
    atd_datetime = fields.Datetime('ATD', help="Actual Time of Departure")
    eta_datetime = fields.Datetime('ETA', help="Estimated Time of Arrival")
    ata_datetime = fields.Datetime('ATA', help="Actual Time of Arrival")

    

    @api.depends('ocean_shipment_mode','air_shipment_mode','road_shipment_mode','rail_shipment_mode')
    ...
    def create(self, values):
        values['name_seq'] = self.env['ir.sequence'].next_by_code('shipment.order')
        if values.get('name', ('New')) == ('New'):
            values['name'] = 'GES/SHO' + values['name_seq']
        result = super(ShipmentOrder, self).create(values)   
        mcroute = self.env['shipment.order.route'].sudo().create({
            'shipment_order_id': result.id,
            'route_type': 'main-carriage',
            'is_main_route': True,
            'is_main_pickup': False,
            'is_main_delivery': False,
            'transport': values.get('transport'),
            'origin_address_type': values.get('origin_address_type'),
            'origin_port_id': values.get('origin_port_id'),
            'origin_address_id': values.get('origin_address_id'),
            'destination_address_type': values.get('destination_address_type'),
            'destination_port_id': values.get('destination_port_id'),
            'destination_address_id': values.get('destination_address_id'),
            'etd_datetime': values.get('etd_datetime'),
            'atd_datetime': values.get('atd_datetime'),
            'eta_datetime': values.get('eta_datetime'),
            'ata_datetime': values.get('ata_datetime')       
        })
        if 'pickup_option' in values:
            pkroute = self.env['shipment.order.route'].sudo().create({
                'shipment_order_id': result.id,
                'route_type': 'pre-carriage',
                'is_main_route': False,
                'is_main_pickup': True,
                'is_main_delivery': False,
                'transport': 'road',
                'origin_address_type': 'location',
                'origin_port_id': False,
                'origin_address_id': values.get('pickup_address_id'),
                'destination_address_type': values.get('origin_address_type'),
                'destination_port_id': values.get('origin_port_id'),
                'destination_address_id': values.get('origin_address_id'),
                'etd_datetime': values.get('pickup_etd_datetime'),
                'atd_datetime': values.get('pickup_atd_datetime'),
                'eta_datetime': values.get('pickup_eta_datetime'),
                'ata_datetime': values.get('pickup_ata_datetime')       
            })
        if 'delivery_option' in values:
            pkroute = self.env['shipment.order.route'].sudo().create({
                'shipment_order_id': result.id,
                'route_type': 'on-carriage',
                'is_main_route': False,
                'is_main_pickup': False,
                'is_main_delivery': True,
                'transport': 'road',
                'origin_address_type': values.get('destination_address_type'),
                'origin_port_id': values.get('destination_port_id'),
                'origin_address_id': values.get('destination_address_id'),
                'destination_address_type': 'location',
                'destination_port_id': False,
                'destination_address_id': values.get('delivery_address_id'),
                'etd_datetime': values.get('delivery_etd_datetime'),
                'atd_datetime': values.get('delivery_atd_datetime'),
                'eta_datetime': values.get('delivery_eta_datetime'),
                'ata_datetime': values.get('delivery_ata_datetime')       
            })
        return result
    ...
    def write(self, values):
        ...
        write_dict = {
            'shipment_order_id': self.id,
        }

        for fn in fields_to_sync:
            if fn in values:
                write_dict[fn] = values.get(fn)

        mc_route_id = self.env['shipment.order.route'].sudo().search([('shipment_order_id','=',self.id),('route_type','=','main-carriage'),('is_main_route','=',True)])     
        if mc_route_id:
            mc_route_id.write(write_dict)  
        else:
            self.env['shipment.order.route'].sudo().create({
            'shipment_order_id': self.id,
            'route_type': 'main-carriage',
            'is_main_route': True,
            'is_main_pickup': False,
            'is_main_delivery': False,
            'transport': values.get('transport') or self.transport,
            'origin_address_type': values.get('origin_address_type') or self.origin_address_type,
            'origin_port_id': values.get('origin_port_id') or self.origin_port_id.id,
            'origin_address_id': values.get('origin_address_id') or self.origin_address_id.id,
            'destination_address_type': values.get('destination_address_type') or self.destination_address_type,
            'destination_port_id': values.get('destination_port_id') or self.destination_port_id.id,
            'destination_address_id': values.get('destination_address_id') or self.destination_address_id.id,
            'etd_datetime': values.get('etd_datetime') or self.etd_datetime,
            'atd_datetime': values.get('atd_datetime') or self.atd_datetime,
            'eta_datetime': values.get('eta_datetime') or self.eta_datetime,
            'ata_datetime': values.get('ata_datetime') or self.ata_datetime
            })

        mp_route_id = self.env['shipment.order.route'].sudo().search([('shipment_order_id','=',self.id),('route_type','=','pre-carriage'),('is_main_pickup','=',True)])     
        if mp_route_id:
            if values.get('pickup_option') == False:
                mp_route_id.unlink()
            else:
                mp_route_id.write({
                'transport': 'road',
                'origin_address_type': 'location',
                'origin_port_id': False,
                'origin_address_id': values.get('pickup_address_id') or self.pickup_address_id.id,
                'destination_address_type': values.get('origin_address_type') or self.origin_address_type,
                'destination_port_id': values.get('origin_port_id') or self.origin_port_id.id,
                'destination_address_id': values.get('destination_address_id') or self.destination_address_id.id,
                'etd_datetime': values.get('pickup_etd_datetime') or self.pickup_etd_datetime,
                'atd_datetime': values.get('pickup_atd_datetime') or self.pickup_atd_datetime,
                'eta_datetime': values.get('pickup_eta_datetime') or self.pickup_eta_datetime,
                'ata_datetime': values.get('pickup_ata_datetime') or self.pickup_ata_datetime
                })  
        else:
            if values.get('pickup_option') == True or self.pickup_option:
                self.env['shipment.order.route'].sudo().create({
                'shipment_order_id': self.id,
                'route_type': 'pre-carriage',
                'is_main_route': False,
        ...
        md_route_id = self.env['shipment.order.route'].sudo().search([('shipment_order_id','=',self.id),('route_type','=','on-carriage'),('is_main_delivery','=',True)])     
        if md_route_id:
            if values.get('delivery_option') == False:
                md_route_id.unlink()
            else:
                md_route_id.write({
                'transport': 'road',
                'origin_address_type': values.get('destination_address_type') or self.destination_address_type,
                'origin_port_id': values.get('destination_port_id') or self.destination_port_id.id,
                'origin_address_id': values.get('destination_address_id') or self.destination_address_id.id,
                'destination_address_type': 'location',
                'destination_port_id': False,
                'destination_address_id': values.get('delivery_address_id') or self.delivery_address_id.id,
                'etd_datetime': values.get('delivery_etd_datetime') or self.delivery_etd_datetime,
                'atd_datetime': values.get('delivery_atd_datetime') or self.delivery_atd_datetime,
                'eta_datetime': values.get('delivery_eta_datetime') or self.delivery_eta_datetime,
                'ata_datetime': values.get('delivery_ata_datetime') or self.delivery_ata_datetime
                })  
        else:
            if values.get('delivery_option') == True or self.delivery_option:
                self.env['shipment.order.route'].sudo().create({
                'shipment_order_id': self.id,
                'route_type': 'on-carriage',
                'is_main_route': False,
    ...
    plan_ids = fields.One2many('shipment.order.plan','shipment_order_id', string="Plan")
    pre_plan_ids = fields.One2many('shipment.order.plan','shipment_order_id', string="Plan")
    on_plan_ids = fields.One2many('shipment.order.plan','shipment_order_id', string="Plan")

    operation_ids = fields.One2many('shipment.order.operation','shipment_order_id', string="Operations")
    ...
    def _compute_routes(self):
        for record in self:
            record.pre_route_ids = record.route_ids.filtered(lambda r: r.route_type == 'pre-carriage')
            record.on_route_ids = record.route_ids.filtered(lambda r: r.route_type == 'on-carriage')
