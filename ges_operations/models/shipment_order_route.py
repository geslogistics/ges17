...
class ShipmentOrderRoute(models.Model):
    ...
    shipment_order_id = fields.Many2one('shipment.order', string="Shipment Order", ondelete="cascade", auto_join=True)
    ...
    route_type = fields.Selection(([('main-carriage','Main-Carriage'),('pre-carriage', 'Pre-Carriage'), ('on-carriage', 'On-Carriage')]), string='Route Type')
    ...
    is_main_route = fields.Boolean()
    is_main_pickup = fields.Boolean()
    is_main_delivery = fields.Boolean()
    
    plan_ids = fields.One2many('shipment.order.plan','route_id')
    ...
    def create(self, values):
        if values.get('route_type') == 'pre-carriage':
            values['type_seq'] = 10

        if values.get('route_type') == 'main-carriage':
            values['type_seq'] = 30

        if values.get('route_type') == 'on-carriage':
            values['type_seq'] = 50
        result = super(ShipmentOrderRoute, self).create(values) 
        
        """
        parent_criteria = [
            ('shipment_order_id','=',values.get('shipment_order_id')),
            ('operation_type','=', 'leg'),
        ...
        parent_plan = self.env['shipment.order.plan'].sudo().search(parent_criteria)
        if parent_plan:
            newchildplan = self.env['shipment.order.plan'].sudo().create({            
            'route_id': result.id,
            'shipment_order_id': values.get('shipment_order_id'),
            ...
            newplan = self.env['shipment.order.plan'].sudo().create({            
            'route_id': result.id,
            'shipment_order_id': values.get('shipment_order_id'),
            'operation_type': 'leg',
            'route_type': values.get('route_type'),
    ...
    def write(self, values):
        ...
        if values.get('route_type') == 'on-carriage':
            values['type_seq'] = 50
        result = super(ShipmentOrderRoute, self).write(values)
        
        """
        for record in self:
            record.plan_ids.unlink()
            ...
            parent_plan = record.env['shipment.order.plan'].sudo().search(parent_criteria)
            if parent_plan:
                newparentplan = record.env['shipment.order.plan'].sudo().create({            
                'route_id': record.id,
                'shipment_order_id': values.get('shipment_order_id'),
                'parent_plan_id': parent_plan.id,
                'operation_type': 'leg',
                ...
                newplan = record.env['shipment.order.plan'].sudo().create({            
                'route_id': record.id,
                'shipment_order_id': values.get('shipment_order_id'),
                'operation_type': 'leg',
                'route_type': values.get('route_type'),
