...
class CMORW(models.TransientModel):
    ...
    assigned_user_id = fields.Many2one('res.users', string="Assigned Buyer", compute="_compute_main_user_team", store=True, readonly=False, index=True, domain = [('current_kyc_state', '=', 'active'),('current_kyc_expiry_date', '>', fields.Date.today()),('current_vrm_state', '=', 'active'),('current_vrm_expiry_date', '>', fields.Date.today())])
...
class CMORWExItem(models.TransientModel):
    ...
    vendor_id = fields.Many2one('res.partner', string="Vendor", domain = [('current_kyc_state', '=', 'active'),('current_kyc_expiry_date', '>', fields.Date.today()),('current_vrm_state', '=', 'active'),('current_vrm_expiry_date', '>', fields.Date.today())])
    assigned_user_id = fields.Many2one(comodel_name='res.users', string="Assigned To", compute='_compute_assigned_user_id', store=True, readonly=False, index=True, domain = [('default_operating_unit_procurement_id','!=',False)])
        ...
        for record in self:
            if record.vendor_id and not (record._origin.id and record.assigned_user_id):
                if record.vendor_id.procurement_user_id:
                    ...
                    record.assigned_user_id = record.assigned_user_id or False
