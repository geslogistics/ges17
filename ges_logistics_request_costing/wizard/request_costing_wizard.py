    ...
    def _compute_assigned_user_id(self):
        for record in self:
            if record.vendor_id and not (record._origin.id and record.assigned_user_id):
                record.assigned_user_id = record.vendor_id.buyer_id or record.assigned_user_id
    ...
    def _compute_assigned_user_id(self):
        for record in self:
            if record.product_id and not (record._origin.id and record.assigned_user_id):
                record.assigned_user_id = record.product_id.buyer_id or record.assigned_user_id