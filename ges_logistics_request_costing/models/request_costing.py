...
class CostingRequest(models.Model):
    ...
    def _action_submit(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing', record.id))),('state','=','open'),('action','in',['req_cost_submit','req_cost_resubmit'])])
    ...
    def _action_validate_sale(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing', record.id))),('state','=','open'),('action','=','req_cost_validate_sale')])
            if related_ticket_ids:
                datas = {
                    'state':'closed',
                    'closed_datetime': fields.Datetime.now(),
    ...
    def _action_submit_selection(self, note=False):
        for record in self:
            ...
            #close current related tickets and add note
            #related_ticket_ids = record.ticket_ids.filtered(lambda r: r.state == 'open' and r.action in ['req_cost_submit','req_cost_resubmit'])
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing', record.id))),('state','=','open'),('action','=','req_cost_select')])
            if related_ticket_ids:
                datas = {
                    'state':'closed',
                    'closed_datetime': fields.Datetime.now(),
    ...
    def _action_cancel(self, note=False):
        for record in self:
            ...
            #close current related tickets and add note
            #related_ticket_ids = record.ticket_ids.filtered(lambda r: r.state == 'open' and r.action in ['req_cost_submit','req_cost_resubmit'])
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing', record.id))),('state','=','open')])
            if related_ticket_ids:
                datas = {
                    'state':'cancel',
                    'closed_datetime': fields.Datetime.now(),
...
class CostingRequestItem(models.Model):
    ...
    def _submit_quote(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing.item', record.id))),('state','=','open'),('action','=','req_cost_cost_item')])
            if related_ticket_ids:
                datas = {
                    'state':'closed',
                    'closed_datetime': fields.Datetime.now(),
    ...
    def _validate_quote(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing.item', record.id))),('state','=','open'),('action','=','req_cost_validate_purchase')])
    ...
    def _validate_failed(self, note=False):
        for record in self:
            ...
            pending_items = record.request_id.request_item_ids.filtered(lambda r: r.state not in ['quoted','failed'])
            #pending_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing', record.request_id.id))),('state','=','open'),('action','in',['req_cost_validate_purchase','req_cost_cost_item'])])
            if not pending_items:
    ...
    def _confirm_quote(self, note=False):
        for record in self:
            ...
            #related_ticket_ids = record.ticket_ids.filtered(lambda r: r.state == 'open' and r.action == 'req_cost_validate_purchase')
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing.item', record.id))),('state','=','open'),('action','=','req_cost_confirm_item')])
    ...
    def _bill_quote(self, note=False):
        for record in self:
            ...
            #related_ticket_ids = record.ticket_ids.filtered(lambda r: r.state == 'open' and r.action == 'req_cost_validate_purchase')
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing.item', record.id))),('state','=','open'),('action','=','req_cost_bill_item')])
    ...
    def _cancel_item(self, note=False):
        for record in self:
            ...
            #related_ticket_ids = record.ticket_ids.filtered(lambda r: r.state == 'open' and r.action == 'req_cost_validate_purchase')
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('request.costing.item', record.id))),('state','=','open')])
