...
class Partner(models.Model):
    ...
    def onchange_operating_unit_ids(self):
        for record in self:
            record.sales_user_id = record.country_id.sales_user_id.id if record.country_id.sales_user_id else None
            record.sales_ops_user_id = record.country_id.sales_ops_user_id.id if record.country_id.sales_ops_user_id else None
            record.procurement_user_id = record.country_id.procurement_user_id.id if record.country_id.procurement_user_id else None
            record.procurement_ops_user_id = record.country_id.procurementops__user_id.id if record.country_id.procurement_ops_user_id else None
            record.finance_user_id = record.country_id.finance_user_id.id if record.country_id.finance_user_id else None
            record.finance_ops_user_id = record.country_id.financeops__user_id.id if record.country_id.finance_ops_user_id else None
            record.hr_user_id = record.country_id.hr_user_id.id if record.country_id.hr_user_id else None
            record.it_user_id = record.country_id.it_user_id.id if record.country_id.it_user_id else None
            record.executive_user_id = record.country_id.executive_user_id.id if record.country_id.executive_user_id else None
    ...
    sales_referral_ou_id = fields.Many2one('operating.unit', string="Sales Referral Unit", related='sales_referral_user_id.default_operating_unit_sales_id', depends=['sales_user_id.default_operating_unit_sales_id'], store=True, index=True) 

    procurement_referral_user_id = fields.Many2one('res.users', string="Procurement Referral User")
    procurement_referral_ou_id = fields.Many2one('operating.unit', string="Procurement Referral Unit", related='procurement_referral_user_id.default_operating_unit_sales_id', depends=['sales_user_id.default_operating_unit_sales_id'], store=True, index=True) 