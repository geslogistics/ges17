...
class PurchaseOrder(models.Model):
    ...
    def _compute_pa_currency_id_domain(self):
        for record in self.sudo():
            current_pa_currency_ids = record.partner_id.current_vrm_id.vendor_currency_ids.ids
    ...
    def _compute_pa_payment_term_id_domain(self):
        for record in self.sudo():
            cash_days = 0
            current_pa_payment_term_ids = record.partner_id.current_vendor_credit_id.vendor_payment_term_ids.ids
    ...
    procurement_user_id = fields.Many2one('res.users', string='Procurement User', compute='_compute_assigned_user_ou', store=True)
    procurement_ou_id = fields.Many2one('operating.unit', string='Procurement Unit', compute='_compute_assigned_user_ou', store=True)
    
    @api.depends('partner_id')
    def _compute_assigned_user_ou(self):
        for record in self:
            if not (record._origin.id and record.procurement_user_id):
                record.procurement_user_id = record.partner_id.procurement_user_id
                record.procurement_ou_id = record.partner_id.procurement_ou_id