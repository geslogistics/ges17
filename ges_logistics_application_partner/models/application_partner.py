...
class ApplicationPartner(models.Model):
    ...
    state_selections = [
        ...
        ('function_validation', 'Function Validation'),
        ('approval', 'Approval'),
        ('approved', 'Approved'), 
        ('rejected', 'Rejected'), 
        ('active', 'Active'),
    ...
    def _domain_partner_id(self):
        if self.env.user.has_group('ges_logistics_application_partner.group_partner_application_user_all_docs'):
            return "[(1, '=', 1)]"

        if self.env.user.has_group('ges_logistics_application_partner.group_partner_application_user_team_docs'):
            return "['|','|','|','&',('sales_ou_id', '=', current_operating_unit_sales_id),('sales_ou_id', '!=', False),'&',('procurement_ou_id', '=', current_operating_unit_procurement_id),('procurement_ou_id', '!=', False),'&',('sales_user_id', '=', current_user_id),('sales_user_id', '!=', False),'&',('procurement_user_id', '=', current_user_id),('procurement_user_id', '!=', False)]"

        return "['|',('sales_user_id', '=', current_user_id),('procurement_user_id', '=', current_user_id)]"
            
    partner_id = fields.Many2one("res.partner", string="Partner", domain=lambda self: self._domain_partner_id())
    requester_user_id = fields.Many2one('res.users', string='Request User', default=lambda self: self.env.user)
    validator_user_ids = fields.Many2many('res.users', 'unit_validator_application', 'application_validator_id' ,'user_application_validator_id', string='Unit Validator User')
    ...
    is_admin = fields.Boolean(compute='_check_admin_group', store=False)
    is_unit_validator = fields.Boolean(compute='_check_unit_validator', store=False)

    def _check_admin_group(self):
        self.is_admin = False
        if self.user_has_groups('ges_logistics_application_partner.group_partner_application_admin'):
            self.is_admin = True
            

    @api.depends('validator_user_ids','current_user_id')
    @api.onchange('validator_user_ids','current_user_id')
    def _check_unit_validator(self):
        self.is_unit_validator = False
        if self.current_user_id in self.validator_user_ids or self.is_admin:
            self.is_unit_validator = True            
    

    # current application fields
    #Application fields
    current_kyc_id = fields.Many2one('application.partner', string="KYC Application", tracking=True, compute="_get_current_applications", store=True)
    ...
    current_crm_id = fields.Many2one('application.partner', string="CRM Application", tracking=True, compute="_get_current_applications", store=True)
    ...
    current_vrm_id = fields.Many2one('application.partner', string="VRM Application", tracking=True, compute="_get_current_applications", store=True)
    ...
    current_customer_credit_id = fields.Many2one('application.partner', string="Customer Credit Application", tracking=True, compute="_get_current_applications", store=True)
    ...
    current_vendor_credit_id = fields.Many2one('application.partner', string="Vendor Credit Application", tracking=True, compute="_get_current_applications", store=True)
    ...
    current_customer_strategy_id = fields.Many2one('application.partner', string="Customer Strategy Credit Application", tracking=True, compute="_get_current_applications", store=True)
    ...
    current_vendor_strategy_id = fields.Many2one('application.partner', string="Vendor Strategy Credit Application", tracking=True, compute="_get_current_applications", store=True)
    ...
    sales_user_id = fields.Many2one('res.users', string='Sales User', compute='_compute_sales_user_id', store=True, readonly=False, index=True)
    ...
    def _compute_sales_user_id(self):
        for record in self:
            record.sales_user_id = False
            if record.partner_id:
                record.sales_user_id = record.partner_id.sales_user_id
    
    @api.depends('sales_user_id','partner_id')
    @api.onchange('sales_user_id','partner_id')
    ...
    sales_ops_user_id = fields.Many2one('res.users', string='Sales Ops User', compute='_compute_sales_ops_user_id', store=True, readonly=False, index=True)
    sales_ops_ou_id = fields.Many2one('operating.unit', string='Sales Ops Unit', compute='_compute_sales_ops_ou_id', store=True, readonly=False, index=True)
    
    @api.depends('partner_id')
    @api.onchange('partner_id')
    def _compute_sales_ops_user_id(self):
        for record in self:
            record.sales_ops_user_id = False
            if record.partner_id:
                record.sales_ops_user_id = record.partner_id.sales_ops_user_id
    
    @api.depends('sales_ops_user_id','partner_id')
    @api.onchange('sales_ops_user_id','partner_id')
    def _compute_sales_ops_ou_id(self):
        for record in self:
            record.sales_ops_ou_id = False
            if record.sales_ops_user_id:
                record.sales_ops_ou_id = record.sales_ops_user_id.default_operating_unit_sales_ops_id
    ...
    # Vendor VRM Fields
    procurement_user_id = fields.Many2one('res.users', string='Procurement User', compute='_compute_procurement_user_id', store=True, readonly=False, index=True)
    ...
    @api.depends('partner_id')
    @api.onchange('partner_id')
    def _compute_procurement_user_id(self):
        for record in self:
            record.procurement_user_id = False
            if record.partner_id:
                record.procurement_user_id = record.partner_id.procurement_user_id
    
    @api.depends('procurement_user_id','partner_id')
    @api.onchange('procurement_user_id','partner_id')
    ...
    procurement_ops_user_id = fields.Many2one('res.users', string='Procurement Ops User', compute='_compute_procurement_ops_user_id', store=True, readonly=False, index=True)
    procurement_ops_ou_id = fields.Many2one('operating.unit', string='Procurement Ops Unit', compute='_compute_procurement_ops_ou_id', store=True, readonly=False, index=True)

    @api.depends('partner_id')
    @api.onchange('partner_id')
    def _compute_procurement_ops_user_id(self):
        for record in self:
            record.procurement_ops_user_id = False
            if record.partner_id:
                record.procurement_ops_user_id = record.partner_id.procurement_ops_user_id
    
    @api.depends('procurement_ops_user_id','partner_id')
    @api.onchange('procurement_ops_user_id','partner_id')
    def _compute_procurement_ops_ou_id(self):
        for record in self:
            record.procurement_ops_ou_id = False
            if record.procurement_ops_user_id:
                record.procurement_ops_ou_id = record.procurement_ops_user_id.default_operating_unit_procurement_ops_id
    ...
    #attachment fields
    attachment_ids = fields.One2many('application.attachment','application_id', string="Attachments")

    #tickets fields
    ticket_ids = fields.One2many('ticket','app_partner_id', string="Tickets")
    ...
    previous_city_id = fields.Many2one(comodel_name='res.city', string='City', related="current_kyc_id.city_id")
    previous_country_enforce_cities = fields.Boolean(related='current_kyc_id.country_enforce_cities')
    ...
    previous_sales_user_id = fields.Many2one('res.users', string='Sales User', related="current_crm_id.sales_user_id")
    previous_sales_ou_id = fields.Many2one('operating.unit', string='Sales Unit', related="current_crm_id.sales_ou_id")
    previous_sales_ops_user_id = fields.Many2one('res.users', string='Sales Ops User', related="current_crm_id.sales_ops_user_id")
    previous_sales_ops_ou_id = fields.Many2one('operating.unit', string='Sales Ops Unit', related="current_crm_id.sales_ops_ou_id")
    ...
    # Vendor VRM Fields
    previous_procurement_user_id = fields.Many2one('res.users', string='Procurement User', related="current_crm_id.procurement_user_id")
    previous_procurement_ou_id = fields.Many2one('operating.unit', string='Procurement Unit', related="current_crm_id.procurement_ou_id")
    previous_procurement_ops_user_id = fields.Many2one('res.users', string='Procurement Ops User', related="current_crm_id.procurement_ops_user_id")
    previous_procurement_ops_ou_id = fields.Many2one('operating.unit', string='Procurement Ops Unit', related="current_crm_id.procurement_ops_ou_id")
    ...
    def create(self, values):
        if values.get('name', ('New')) == ('New'):
            #create name
            application_re = "GES/APP"
            values['name'] = application_re + self.env['ir.sequence'].next_by_code('application.partner') or _('New')
            
            

        if (values.get('vat') or self.vat) and (values.get('country_id') or self.country_id):
            vat = values.get('vat') or self.vat
        ...
        result = super(ApplicationPartner, self).create(values)
    ...
    def _copy_previous_kyc_address_info(self):
        ...
        self.city_id = self.previous_city_id.id
    ...
    def _copy_previous_crm_segmentation_info(self):
        self.sales_user_id = self.previous_sales_user_id.id
        self.sales_ops_user_id = self.previous_sales_ops_user_id.id
    ...
    def _copy_previous_vrm_segmentation_info(self):
        self.procurement_user_id = self.previous_procurement_user_id.id
        self.procurement_ops_user_id = self.previous_procurement_ops_user_id.id
    ...
    def _copy_partner_crm_to_new(self):
        self.sales_user_id = self.partner_id.sales_user_id.id
        self.sales_ops_user_id = self.partner_id.sales_ops_user_id.id
    ...
    def _copy_partner_vrm_to_new(self):
        self.procurement_user_id = self.partner_id.procurement_user_id.id
        self.procurement_ops_user_id = self.partner_id.procurement_ops_user_id.id
    ...
    def _action_activate(self):
        for record in self:
            ...
            current_active_application = record.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=',record.application_type),('state','=','active'),('id','!=',record.id)])
            current_active_application.write({'state': 'approved'})
            ...
            if record.application_type == "kyc":                
                ...
                partner_rec.partner_id.city_id = record.city_id.id
            ...
            if record.application_type == "crm":
                partner_rec.partner_id.sales_user_id = record.sales_user_id.id
                partner_rec.partner_id.sales_ops_user_id = record.sales_ops_user_id.id

                if record.partner_id.property_product_pricelist:
                    A = record.partner_id.property_product_pricelist.id
                    B = record.customer_pricelist_ids.ids
                    #raise UserError (A not in B)
                    if record.partner_id.property_product_pricelist.id not in record.customer_pricelist_ids.ids:
                        partner_rec.partner_id.property_product_pricelist = record.customer_pricelist_ids[0]
                        #raise UserError(partner_rec.partner_id.property_product_pricelist)
            ...
            if record.application_type == "vrm":
                partner_rec.partner_id.procurement_user_id = record.procurement_user_id.id
                partner_rec.partner_id.procurement_ops_user_id = record.procurement_ops_user_id.id

                if record.partner_id.property_purchase_currency_id:
                    if record.partner_id.property_purchase_currency_id.id not in record.vendor_currency_ids.ids:
    ...
    def action_wizard_submit(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'res_model': 'application.action.wizard',
            'type': 'ir.actions.act_window',
            'target': 'new',
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_submit'
                }
        }

    def _action_submit(self, note=False):
        for record in self:
            record.write({'state': 'unit_validation'})
            record.requester_user_id = self.env.user.id
            validators = []
            if record.application_type == 'kyc':
                if record.requester_user_id.default_operating_unit_sales_id:
                    validators.append(record.requester_user_id.default_operating_unit_sales_id.manager_id.id)
                if record.requester_user_id.default_operating_unit_procurement_id:
                    validators.append(record.requester_user_id.default_operating_unit_procurement_id.manager_id.id)
            elif record.application_type in ['crm','customer_credit','customer_strategy']:
                if record.requester_user_id.default_operating_unit_sales_id:
                    validators.append(record.requester_user_id.default_operating_unit_sales_id.manager_id.id)
            elif record.application_type in ['vrm','vendor_credit','vendor_strategy']:
                if record.requester_user_id.default_operating_unit_procurement_id:
                    validators.append(record.requester_user_id.default_operating_unit_procurement_id.manager_id.id)

            if validators:
                record.validator_user_ids = [(6, 0, validators)]
            else:
                record.validator_user_ids = [(6, 0, [self.env.user.id])]
                    
            


            #close current related tickets and add note
            #related_ticket_ids = record.ticket_ids.filtered(lambda r: r.state == 'open' and r.action in ['app_partner_submit','app_partner_resubmit'])
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','in',['app_partner_submit','app_partner_resubmit'])])
            ...
            # add pending tickets for requester_ou_id team leader
            user_ids = False
            if record.requester_user_id.default_operating_unit_sales_id:
                user_ids = record.requester_user_id.default_operating_unit_sales_id.manager_id.id
            if record.requester_user_id.default_operating_unit_procurement_id:
                user_ids += record.requester_user_id.default_operating_unit_procurement_id.manager_id.id
            
            user_records = False
            if user_ids:
                user_records = self.env['res.users'].search([('id','in',[user_ids])])
            
            if user_records:
                for user in user_records:
                    datas = {
                        'reference_document': ('%s,%s' % ('application.partner', record.id)),
                        'app_partner_id': record.id,
                        'requester_user_id': self.env.user.id,
                        'requester_note': note,
                        'user_id': user.id,                        
                        'action':'app_partner_validate_unit',
    ...
    def action_wizard_validate_unit(self):
        ...
        return {
            ...
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_validate_unit'
                }
        }

    def _action_validate_unit(self, note=False):
        for record in self:
            record.write({'state': 'function_validation'})
            
            #close current related tickets and add note
            #related_ticket_ids = record.ticket_ids.filtered(lambda r: r.state == 'open' and r.action == 'app_partner_validate_sale')
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','=','app_partner_validate_unit')])
            if related_ticket_ids:
                datas = {
                    'state':'closed',
                    'closed_datetime': fields.Datetime.now(),
            ...
            # add pending actions for Function Validators

            with_user = self.env['ir.config_parameter'].sudo()
            user_ids = False
            if record.application_type == "kyc":
                user_ids = with_user.get_param('ges_logistics_application_partner.partner_kyc_validator_ids')
            elif record.application_type == "crm":
                user_ids = with_user.get_param('ges_logistics_application_partner.partner_crm_validator_ids')
            elif record.application_type == "customer_credit":
                user_ids = with_user.get_param('ges_logistics_application_partner.partner_customer_credit_validator_ids')
            elif record.application_type == "customer_strategy":
                user_ids = with_user.get_param('ges_logistics_application_partner.partner_customer_strategy_validator_ids')
            elif record.application_type == "vrm":
                user_ids = with_user.get_param('ges_logistics_application_partner.partner_vrm_validator_ids')
            elif record.application_type == "vendor_credit":
                user_ids = with_user.get_param('ges_logistics_application_partner.partner_vendor_credit_validator_ids')
            elif record.application_type == "vendor_strategy":
                user_ids = with_user.get_param('ges_logistics_application_partner.partner_vendor_strategy_validator_ids')
            ...
            if user_records:
                for user in user_records:
                    ...
                    datas = {
                        ...
                        'requester_note': note,
                        'user_id': user.id,
                        'action':'app_partner_validate_function',
                        'opened_datetime': fields.Datetime.now(),
                        'state':'open',
                    }
                    new_ticket = self.env['ticket'].sudo().create(datas)
    ...
    def action_wizard_return_unit(self):
        ...
        return {
            ...
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_return_unit'
                }
        }

    def _action_return_unit(self, note=False):
        for record in self:
            ...
            user_ids = related_ticket_ids.requester_user_id
            if user_ids:
    ...
    def action_wizard_validate_function(self):
        ...
        return {
            ...
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_validate_function'
                }
        }

    def _action_validate_function(self, note=False):
        for record in self:
            ok_to_proceed = False
            
            #close current user related tickets and add note
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('user_id','=',self.env.user.id),('state','=','open'),('action','=','app_partner_validate_function')])
            ...
            if related_ticket_ids:
                ...
                raise UserError("You cannot perform this action. Either already done or another user is required. (Error: No pending tickets)")
                
            # change application status and/or other user actions depending on Application Type Decision Making - signly or jointly
            with_user = self.env['ir.config_parameter'].sudo()
            decision_making = False
            if record.application_type == "kyc":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_kyc_validator_ids_decision')
            elif record.application_type == "crm":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_crm_validator_ids_decision')
            elif record.application_type == "customer_credit":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_customer_credit_validator_ids_decision')
            elif record.application_type == "customer_strategy":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_customer_strategy_validator_ids_decision')
            elif record.application_type == "vrm":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_vrm_validator_ids_decision')
            elif record.application_type == "vendor_credit":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_vendor_credit_validator_ids_decision')
            elif record.application_type == "vendor_strategy":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_vendor_strategy_validator_ids_decision')

            if decision_making == 'singly':
                ok_to_proceed = True
                related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','=','app_partner_validate_function')])
            ...
            if ok_to_proceed:
                ...
                # add tickets for approvers
                
                with_user = self.env['ir.config_parameter'].sudo()
                user_ids = False
                if record.application_type == "kyc":
                    user_ids = with_user.get_param('ges_logistics_application_partner.partner_kyc_approver_ids')
                elif record.application_type == "crm":
                    user_ids = with_user.get_param('ges_logistics_application_partner.partner_crm_approver_ids')
                elif record.application_type == "customer_credit":
                    user_ids = with_user.get_param('ges_logistics_application_partner.partner_customer_credit_approver_ids')
                elif record.application_type == "customer_strategy":
                    user_ids = with_user.get_param('ges_logistics_application_partner.partner_customer_strategy_approver_ids')
                elif record.application_type == "vrm":
                    user_ids = with_user.get_param('ges_logistics_application_partner.partner_vrm_approver_ids')
                elif record.application_type == "vendor_credit":
                    user_ids = with_user.get_param('ges_logistics_application_partner.partner_vendor_credit_approver_ids')
                elif record.application_type == "vendor_strategy":
                    user_ids = with_user.get_param('ges_logistics_application_partner.partner_vendor_strategy_approver_ids')
    ...
    def action_wizard_return_function(self):
        ...
        return {
            ...
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_return_function'
                }
        } 

    def _action_return_function(self, note=False):
        for record in self:  
            record.write({'state': 'draft'})
            ...
            # add pending tickets for requester user_id
            user_ids = related_ticket_ids.requester_user_id.id
            if user_ids:
                for user in user_ids:
                    datas = {
                        'reference_document': ('%s,%s' % ('application.partner', record.id)),
    ...
    def action_wizard_approve(self):
        ...
        return {
            ...
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_approve'
                }
        } 
    ...
    def _action_approve(self, note=False):
        for record in self:
            ...
            #close current user related tickets and add note
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('user_id','=',self.env.user.id),('state','=','open'),('action','=','app_partner_approve')])
            ...
            # change application status and/or other tickets depending on Application Type Decision Making - signly or jointly
            with_user = self.env['ir.config_parameter'].sudo()
            decision_making = False
            if record.application_type == "kyc":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_kyc_approver_ids_decision')
            elif record.application_type == "crm":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_crm_approver_ids_decision')
            elif record.application_type == "customer_credit":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_customer_credit_approver_ids_decision')
            elif record.application_type == "customer_strategy":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_customer_strategy_approver_ids_decision')
            elif record.application_type == "vrm":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_vrm_approver_ids_decision')
            elif record.application_type == "vendor_credit":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_vendor_credit_approver_ids_decision')
            elif record.application_type == "vendor_strategy":
                decision_making = with_user.get_param('ges_logistics_application_partner.partner_vendor_strategy_approver_ids_decision')
            
            if decision_making == 'singly':
                ok_to_proceed = True
                related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','=','app_partner_approve')])
    ...
    def action_wizard_reject(self):
        ...
        return {
            ...
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_reject'
                }
        } 
    ...
    def action_wizard_cancel(self):
        ...
        return {
            ...
            'context': {
                'default_application_ids': self.ids,
                'default_name': 'app_partner_cancel'
                }
        } 
    ...
    def _action_cancel(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].sudo().search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open')])
    ...
    def write(self, values):
        ...
        if 'state' in values:
            ...
            if new_state == 'draft' and self.state != 'draft':
                values['color'] = False
                if not self.env.user.has_group(
                        'ges_logistics_application_partner.group_partner_application_user_return_draft'):
                    raise ValidationError(
                        _("Only Manager can perform that move!"))
        ...
        if 'active' in values:
            if not self.env.user.has_group(
                    'ges_logistics_application_partner.group_partner_application_user_archive'):
                raise ValidationError(
                    _("Only Managers can perform that move!"))
    ...
    def _get_current_applications(self):
        for record in self:
            current_kyc = self.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=','kyc'),('state','in',['active','expired'])])
            record.current_kyc_id = current_kyc[0] if current_kyc else False

            current_crm = self.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=','crm'),('state','in',['active','expired'])])
            record.current_crm_id = current_crm[0] if current_crm else False

            current_customer_credit = self.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=','customer_credit'),('state','in',['active','expired'])])
            record.current_customer_credit_id = current_customer_credit[0] if current_customer_credit else False

            current_customer_strategy = self.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=','customer_strategy'),('state','in',['active','expired'])])
            record.current_customer_strategy_id = current_customer_strategy[0] if current_customer_strategy else False

            current_vrm = self.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=','vrm'),('state','in',['active','expired'])])
            record.current_vrm_id = current_vrm[0] if current_vrm else False

            current_vendor_credit = self.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=','vendor_credit'),('state','in',['active','expired'])])
            record.current_vendor_credit_id = current_vendor_credit[0] if current_vendor_credit else False

            current_vendor_strategy = self.env['application.partner'].search([('partner_id','=',record.partner_id.id),('application_type','=','vendor_strategy'),('state','in',['active','expired'])])
    ...
    def _get_request_type_options(self):
        ...
        if self.partner_id:
            ...
            sale_user_not = self.partner_id.sales_user_id != self.current_user_id and self.partner_id.sales_user_id.id != False
            sale_team_not = self.partner_id.sales_ou_id != self.current_operating_unit_sales_id and self.partner_id.sales_ou_id.id != False
            all_sale_not = sale_user_not and sale_team_not
            
            purchase_user_not = self.partner_id.procurement_user_id != self.current_user_id and self.partner_id.procurement_user_id.id != False
            purchase_team_not = self.partner_id.procurement_ou_id != self.current_operating_unit_procurement_id and self.partner_id.procurement_ou_id.id != False
            ...
            if self.env.user.has_group('ges_logistics_application_partner.group_partner_application_user_all_docs'):
                A = 1
            elif self.env.user.has_group('ges_logistics_application_partner.group_partner_application_user_team_docs'):
                if is_kyc:
                    ...
                    if purchase_team_not:
                        raise UserError("You do not have access to this record.")
            elif self.env.user.has_group('ges_logistics_application_partner.group_partner_application_user_own_docs'):
                if is_kyc:
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        hide_list = [
            ...
            'previous_city',
            'previous_city_id',
            'previous_country_enforce_cities',
            'previous_state_id',
            'previous_country_id',
...
class ApplicationPartnerUserAction(models.Model):
    _name = 'application.partner.user.action'
    ...
    application_id = fields.Many2one('application.partner', string="Application", no_sort=True)
