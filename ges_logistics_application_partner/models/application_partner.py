...
class ApplicationPartner(models.Model):
    ...
    sales_user_id = fields.Many2one('res.users', string='Sales User', compute='_compute_assigned_user_id', store=True, readonly=False, index=True)
    sales_ou_id = fields.Many2one('operating.unit', string='Sales Unit', compute='_compute_assigned_user_id', store=True, readonly=False, index=True)

    @api.depends('partner_id')
    @api.onchange('partner_id')
    def _compute_sales_user_id(self):
        for record in self:
            if record.partner_id and not (record._origin.id and record.sales_user_id):
                record.sales_user_id = record.partner_id.sales_user_id

            if record.partner_id and not (record._origin.id and record.sales_ou_id):
                record.sales_ou_id = record.partner_id.sales_ou_id
    ...
    procurement_ou_id = fields.Many2one('operating.unit', string='Procurement Unit', related="procurement_user_id.procurement_ou_id", store=True)
    ...
    #user action fields
    user_action_ids = fields.One2many('application.partner.user.action','application_id', string="User Actions")
    ...
    def _action_submit(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','in',['app_partner_submit','app_partner_resubmit'])])
    ...
    def _action_validate_unit(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','=','app_partner_validate_unit')])
            if related_ticket_ids:
                datas = {
                    'state':'closed',
                    'closed_datetime': fields.Datetime.now(),
            ...
            if user_records:
                for user in user_records:
                    ...
                    datas = {
                        'reference_document': ('%s,%s' % ('application.partner', user.id)),
                        'app_partner_id': record.id,
                        'requester_user_id': self.env.user.id,
                        'user_id': user.id,
    ...
    def _action_validate_function(self, note=False):
        for record in self:
            ...
            related_ticket_ids = self.env['ticket'].search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('user_id','=',self.env.user.id),('state','=','open'),('action','=','app_partner_validate_function')])
            ...
            if decision_making == 'singly':
                ok_to_proceed = True
                related_ticket_ids = self.env['ticket'].search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','=','app_partner_validate_function')])
    ...
    def _action_approve(self, note=False):
        for record in self:
            ...
            if decision_making == 'singly':
                ok_to_proceed = True
                related_ticket_ids = self.env['ticket'].search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open'),('action','=','app_partner_approve')])
    ...
    def _action_cancel(self, note=False):
        for record in self:
            # add action for current user(s)
            related_ticket_ids = self.env['ticket'].search([('reference_document','=',('%s,%s' % ('application.partner', record.id))),('state','=','open')])
    ...
    def _get_request_type_options(self):
        ...
        if self.partner_id:
            ...
            sale_user_not = self.partner_id.sales_user_id != self.current_user_id and self.partner_id.sales_user_id != False
            sale_team_not = self.partner_id.sales_ou_id != self.current_operating_unit_sales_id and self.partner_id.sales_ou_id != False
            all_sale_not = sale_user_not and sale_team_not
            
            purchase_user_not = self.partner_id.procurement_user_id != self.current_user_id and self.partner_id.procurement_user_id != False
            purchase_team_not = self.partner_id.procurement_ou_id != self.current_operating_unit_procurement_id and self.partner_id.procurement_ou_id != False
