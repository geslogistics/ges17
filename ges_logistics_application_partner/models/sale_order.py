...
class SaleOrder(models.Model):
    ...
    sales_user_id = fields.Many2one('res.users', string='Sales User', compute='_compute_assigned_user_ou', store=True)
    sales_ou_id = fields.Many2one('operating.unit', string='Sales Unit', compute='_compute_assigned_user_ou', store=True)
    
    @api.depends('partner_id')
    def _compute_assigned_user_ou(self):
        for record in self:
            if not (record._origin.id and record.sales_user_id):
