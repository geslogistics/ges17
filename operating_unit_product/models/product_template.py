...
class ProductTemplate(models.Model):
    ...
    def onchange_operating_unit_ids(self):
        for record in self:
            record.sales_user_id = record.categ_id.sales_user_id.id if record.categ_id.sales_user_id else None
            record.sales_ops_user_id = record.categ_id.sales_ops_user_id.id if record.categ_id.sales_ops_user_id else None
            record.procurement_user_id = record.categ_id.procurement_user_id.id if record.categ_id.procurement_user_id else None
            record.procurement_ops_user_id = record.categ_id.procurementops__user_id.id if record.categ_id.procurement_ops_user_id else None
            record.finance_user_id = record.categ_id.finance_user_id.id if record.categ_id.finance_user_id else None
            record.finance_ops_user_id = record.categ_id.financeops__user_id.id if record.categ_id.finance_ops_user_id else None
            record.hr_user_id = record.categ_id.hr_user_id.id if record.categ_id.hr_user_id else None
            record.it_user_id = record.categ_id.it_user_id.id if record.categ_id.it_user_id else None
            record.executive_user_id = record.categ_id.executive_user_id.id if record.categ_id.executive_user_id else None
